...
def check(cur, visited):  # 최단거리에서 다음길 찾기
    nlist = []
    dir = [[1, 0], [0, 1], [-1, 0], [0, -1]]  # 아래,오른,위,왼
    for i in range(4):
        row = cur[0]+dir[i][0]
...
def astar(info):
    ...
    # 시작점 방문
    o_check = [start]  # 이동가능 좌표들
    o = [start+[0, 0, 0, -1, -1]]  # y좌표, x좌표, F socre, G, H, 부모y, 부모x
    c_check = []  # 끝난 좌표들
    c = []
...
# 출구까지 가장 짧은 이동횟수(Int16), 출구를 찾을때까지 탐색한 좌표의 개수(Int16), 알고리즘 수행시간(Float32, ms))
#  shortest / coor / atime
